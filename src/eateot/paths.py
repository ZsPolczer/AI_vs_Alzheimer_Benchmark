...
# Every chart image generated by the study/report apps. Kept in one place so
# ``clear_charts`` (the lab's CLEAR GRAPHS action) can wipe all graphs at once.
CHART_FILES = [
    IMG_FILE,
    COMPARISON_PNG,
    RESTORE_PNG,
...
def clear_charts() -> list[str]:
    """Delete every generated chart image in ``DATA_DIR``.

    Returns the paths actually removed so callers can report them. Telemetry
    logs and report files (``.md``/``.json``) are left untouched — re-running a
    study regenerates its chart from scratch.
    """
    removed = []
